...
// AOS (para refrescar cuando monta el carrusel)
import AOS from 'aos';
...
const imagesMap = { arepaPlainGrande, arepaPlainPequena, arepaChocolo, arepaQueso, deditosQueso };
...
const ProductCarousel = () => {
  ...
  return (
    <section
      ...
      data-aos="fade-up"                 // <- anima la sección al entrar en viewport
      data-aos-offset="120"
    >
      <div className="mx-auto max-w-6xl px-4">
        ...
        <div role="status" aria-live="polite" className="min-h-[0.5rem]">
          {confirmationMessage && (
            <div
              className="mt-3 rounded-lg bg-green-50 text-green-700 px-4 py-2 shadow"
              data-aos="fade-down"
            >
              {confirmationMessage}
            </div>
        ...
        <div className="mt-8">
          <Swiper
            ...
            className="!pb-10"
            onInit={() => {
              // refresca posiciones de AOS cuando el carrusel queda listo
              ...
              return (
                <SwiperSlide key={p.id} style={{ width: 'min(380px, 90vw)' }}>
                  <div
                    ...
                    data-aos="fade-up"                 // <- anima cada tarjeta
                      ...
                      <div className="p-5">
                        ...
                        <div className="mt-4 flex items-center gap-3">
                          <div className="flex items-center rounded-full border border-gray-300 px-1.5">
                            <button type="button" onClick={() => dec(p.id)} className="h-9 w-9 grid place-content-center text-lg rounded-full hover:bg-gray-50">–</button>
                            <input
                              inputMode="numeric" pattern="[0-9]*" type="number" min="1" max="999"
                              value={qty} onChange={(e) => setQuantities((q)=>({ ...q, [p.id]: Math.max(1, parseInt(e.target.value,10) || 1) }))}
                              className="w-12 text-center bg-transparent focus:outline-none"
                            />
                            <button type="button" onClick={() => inc(p.id)} className="h-9 w-9 grid place-content-center text-lg rounded-full hover:bg-gray-50">+</button>
                          </div>
                        ...
                        {cart.length > 0 && (
                          <div className="mt-3 grid grid-cols-2 gap-2">
                            <Link to="/checkout"><button className="w-full rounded-full bg-amber-400 px-4 py-2 font-semibold text-black hover:shadow-xl">{t('proceed_to_checkout')}</button></Link>
                            <Link to="/cart"><button className="w-full rounded-full bg-black px-4 py-2 font-semibold text-white hover:shadow-xl">{t('view_cart')}</button></Link>
                          </div>
                        )}
                      </div>
                    </article>
